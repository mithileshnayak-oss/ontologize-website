...
/**
 * Aavya approved logo system.
 *
 * Variants (per brand guidelines):
 *   - "dark"  → white + grey mark, white wordmark  (for dark backgrounds)
 *   - "light" → navy + cyan mark, navy wordmark    (for light backgrounds)
 *   - "mono"  → same as dark (single-color output)
 *
 * When no variant is passed, it auto-detects from the current theme.
 */
...
/* ------------------------------------------------------------------ */
/*  Mark — the two-part "A" symbol                                    */
/* ------------------------------------------------------------------ */

const markColors: Record<Variant, { upper: string; lower: string }> = {
  dark:  { upper: "#FFFFFF", lower: "#3AADDD" },
  mono:  { upper: "#FFFFFF", lower: "#ABABAB" },
  light: { upper: "#252940", lower: "#3AADDD" },
};

export function AavyaMark({
  size = 40,
  variant = "dark",
}: {
  size?: number;
...
/* ------------------------------------------------------------------ */
/*  Wordmark — "AAVYA" letterforms                                    */
/* ------------------------------------------------------------------ */

const wordmarkColors: Record<Variant, string> = {
  dark:  "#FFFFFF",
  mono:  "#FFFFFF",
  light: "#252940",
};

export function AavyaWordmark({
  className = "",
  variant = "dark",
}: {
  className?: string;
  ...
  const fill = wordmarkColors[variant];
  return (
    <svg
      viewBox="153 43 360 47"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      aria-label="AAVYA"
      className={className}
    >
      <path fillRule="evenodd" clipRule="evenodd" d="M204.579 86.7904C204.886 87.3841 204.841 88.0965 204.477 88.6685C204.124 89.2406 203.475 89.5861 202.793 89.5861H197.455C196.442 89.5861 195.509 89.0248 195.065 88.1397L179.189 56.6105L163.233 88.1397C162.789 89.0248 161.867 89.5861 160.854 89.5861H155.573C154.891 89.5861 154.253 89.2406 153.889 88.6685C153.525 88.0965 153.491 87.3841 153.798 86.7904L174.363 46.4319C175.274 44.6509 177.14 43.5176 179.189 43.5176C181.226 43.5176 183.092 44.6509 184.003 46.4319L204.579 86.7904Z" fill={fill} />
      <path fillRule="evenodd" clipRule="evenodd" d="M286.361 86.7904C286.668 87.3841 286.634 88.0965 286.27 88.6685C285.906 89.2406 285.269 89.5861 284.586 89.5861H279.248C278.224 89.5861 277.302 89.0248 276.858 88.1397L260.971 56.6105L245.026 88.1397C244.583 89.0248 243.661 89.5861 242.648 89.5861H237.367C236.684 89.5861 236.035 89.2406 235.671 88.6685C235.318 88.0965 235.284 87.3841 235.58 86.7904L256.157 46.4319C257.067 44.6509 258.933 43.5176 260.971 43.5176C263.019 43.5176 264.886 44.6509 265.785 46.4319L286.361 86.7904Z" fill={fill} />
      <path fillRule="evenodd" clipRule="evenodd" d="M307.405 46.313C307.097 45.7194 307.131 45.007 307.496 44.4349C307.86 43.8628 308.497 43.5176 309.18 43.5176H314.529C315.541 43.5176 316.464 44.0787 316.908 44.9638L332.795 76.4929L348.739 44.9638C349.183 44.0787 350.105 43.5176 351.118 43.5176H356.398C357.093 43.5176 357.73 43.8628 358.094 44.4349C358.447 45.007 358.492 45.7194 358.185 46.313L337.609 86.6718C336.699 88.4528 334.832 89.5861 332.795 89.5861C330.747 89.5861 328.88 88.4528 327.981 86.6718L307.405 46.313Z" fill={fill} />
      <path fillRule="evenodd" clipRule="evenodd" d="M389.312 46.5075C388.902 45.9031 388.879 45.1474 389.232 44.5213C389.596 43.9061 390.291 43.5176 391.03 43.5176H396.117C397.039 43.5176 397.893 43.9492 398.405 44.6831L414.588 67.9658L430.692 44.6831C431.193 43.9492 432.058 43.5176 432.98 43.5176H438.135C438.875 43.5176 439.569 43.9061 439.933 44.5213C440.286 45.1474 440.263 45.9031 439.854 46.5075L419.528 76.6334V84.9123C419.528 87.492 417.32 89.5861 414.6 89.5861C411.869 89.5861 409.661 87.492 409.661 84.9123V76.6334L389.312 46.5075Z" fill={fill} />
      <path fillRule="evenodd" clipRule="evenodd" d="M511.792 86.7904C512.099 87.3841 512.065 88.0965 511.7 88.6685C511.336 89.2406 510.699 89.5861 510.005 89.5861H504.667C503.655 89.5861 502.733 89.0248 502.277 88.1397L486.663 56.6105L470.445 88.1397C470.001 89.0248 469.08 89.5861 468.067 89.5861H462.798C462.103 89.5861 461.466 89.2406 461.102 88.6685C460.737 88.0965 460.703 87.3841 461.01 86.7904L481.849 46.4319C482.759 44.6509 484.626 43.5176 486.663 43.5176C488.711 43.5176 490.578 44.6509 491.488 46.4319L511.792 86.7904Z" fill={fill} />
    </svg>
  );
}
